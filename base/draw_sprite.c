...
static t_spr		*get_dist_seen_spr(t_all *all, t_pl *ray)
{
	...
	all->spr_free = visible_spr;
	i = -1;
	j = 0;
	while (++i < all->parsed.amt_spr)
	{
		calculate_angle_btw_pl_and_spr(all, ray, i);
		if (fabs(all->sprite[i].ang_btw_pl_sp) < (VIEV_ANGLE + 0.30) / 2.0)
		{
					...
					t_spr *visible_spr, int i)
{
	...
	data.put_down = (((float)all->parsed.res.height
			- (float)all->parsed.res.height * 0.9)
			/ (visible_spr[i].len_till_pl / all->parsed.res.ratio));
	data.spr_ceiling = (float)all->parsed.res.height / 2.0
			- all->parsed.res.height / ((visible_spr[i].len_till_pl
			* WALL_SCALE) / all->parsed.res.ratio)
			+ data.put_down;
	...
	data.aspect_ratio = (float)all->texture[TEXTURES - 1].t_h
			/ (float)all->texture[TEXTURES - 1].t_w;
	...
	data.ratio_h = (float)all->texture[TEXTURES - 1].t_h / data.spr_height;
	data.ratio_w = (float)all->texture[TEXTURES - 1].t_w / data.spr_width;
	return (data);
}
					...
					t_spr *visible_spr, int i, float *rays)
{
	...
	if (fabs(visible_spr[i].ang_btw_pl_sp) < (VIEV_ANGLE + 0.30) / 2.0)
	{
